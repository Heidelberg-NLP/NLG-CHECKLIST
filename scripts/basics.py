...
def make_tmp(pairs, nl="", prefix=None):
    with tempfile.NamedTemporaryFile(delete=False, mode="w", prefix=prefix) as tmp:
        tmp1 = tmp.name
        for line in pairs[0]:
            tmp.write(line+nl)
    with tempfile.NamedTemporaryFile(delete=False, mode="w", prefix=prefix+"_2") as tmp:
        tmp2 = tmp.name
        for line in pairs[1]:
            tmp.write(line+nl)
