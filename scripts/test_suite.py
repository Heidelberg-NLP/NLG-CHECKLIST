...
def normalize_score(met, average, all_average_dict):
	if met in all_average_dict:
		all_average_dict[met].append((round(float(average[met.strip()][0]), 3), round(float(average[met.strip()][1]), 2)))
	else:
		all_average_dict[met] = [(round(float(average[met.strip()][0]), 3), round(float(average[met.strip()][1]), 2))]

	return all_average_dict
...
if __name__ == "__main__":
	...
	def_metric_dict = read_json("../data/all_metric_scores.json")

	both = wanted + your_wanted

	try:
		if sys.argv[1] == '-m':
			your_scores, your_wanted = {}, []
		else:
			try:
	...
	for phenomenon in sorted(test_cases.keys()):
		...
		for ss, ids in test_cases[phenomenon].items():
			...
			correlation, average = compute_av_scores(ids, wanted, your_wanted, metric_dict, ss)
			...
			if ss == "sick":
				...
				for met in both:
					all_average_sick = normalize_score(met, average, all_average_sick)
				...
				for met in both:
					all_average = normalize_score(met, average, all_average)
			...
			total = [vals[idx][0] for idx in ids]
