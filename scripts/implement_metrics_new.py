...
def compute_smatch(pairs, path, s2=False):
	...
	if s2:
		try:
			smatch_score = subprocess.check_output(["python3", path, "-f", tmp1, tmp2, "-cutoff", "0.9", "-diffsense", "0.95", "-vectors", "vectors/glove.6B.300d.txt", "--ms"])
	...
	try:
		return [float(smatch.split()[3].strip()) for smatch in smatch_score.split('\n')]
	except IndexError:
		return [smatch for smatch in smatch_score.split('\n')]

	# for i, sent in enumerate(pairs[0]):
		# tmp1, tmp2 = make_tmp([["".join(sent)], ["".join(pairs[1][i])]], nl="\n")
		# if s2:
			# try:
				# smatch_score = subprocess.check_output(["python3", path, "-f", tmp1, tmp2, "-cutoff", "0.9", "-diffsense", "--ms"])
			# except Exception as e:
				# print(e)
				# print(sent)
				# smatch_score = "nan"
